...
def test_parser(file_path: str):
    ...
    start = time.time()
    try:
        logger.info(f"Initializing parser...")
        ...
        if len(document.content) > 1000000:  # 1MB of text
            logger.warning(f"⚠ WARNING: Very large document content: {len(document.content)} characters")
...
def main():
    ...
    if doc_id:
        logger.info(f"✓ Document successfully processed and stored!")
        logger.info(f"  Document ID: {doc_id}")
    else:
